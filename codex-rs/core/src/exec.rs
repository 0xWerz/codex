...
pub(crate) async fn execute_exec_env(
    ...
    let telemetry_decision = match network.as_ref() {
        Some(network) => {
            blocking_network_policy_decision_from_blocked_queue(network, started_at_unix).await
        }
        None => None,
    };
    match finalized {
...
async fn blocking_network_policy_decision_from_blocked_queue(
    ...
    let blocked = network.drain_blocked_requests().await.ok()?;
    blocked
        .into_iter()
        .rev()
        .find_map(|entry| network_policy_decision_from_blocked_entry(&entry, started_at_unix))
}
...
fn network_policy_decision_from_blocked_entry(
    ...
    if entry.timestamp < started_at_unix {
        return None;
    }
...
mod tests {
    ...
    fn network_policy_decision_maps_fresh_entries() {
        ...
        let payload = network_policy_decision_from_blocked_entry(&entry, 199)
    ...
    fn network_policy_decision_ignores_allow_entries() {
        ...
        assert_eq!(
            network_policy_decision_from_blocked_entry(&entry, 199),
            None
        );
    }

    #[test]
    fn network_policy_decision_ignores_stale_entries() {
        let entry = BlockedRequest {
            host: "google.com".to_string(),
            reason: "not_allowed".to_string(),
            client: None,
        ...
        assert_eq!(
            network_policy_decision_from_blocked_entry(&entry, 101),
            None
        );
    }
